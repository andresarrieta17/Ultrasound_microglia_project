...
RunAzimuth.Seurat <- function(
  ...
  require(dplyr)
  dims <- as.double(length(slot(reference, "reductions")$pca))
...
#' @param pca Name of DimReduc in reference to use for mapping
...
AzimuthReference <- function(
  ...
  if (!pca %in% Reductions(object = object)) {
    stop("pca (", pca, ") not found in Seurat object provided")
  }
  ...
  suppressWarnings(expr = object[["pca"]] <- object[[pca]])
  ...
  features <- rownames(x = Loadings(object = object[['pca']]))
  ...
  DefaultAssay(object = object[["pca"]]) <- refAssay
  object <- DietSeurat(
    object = object,
    counts = FALSE,
    assays = c(refAssay, assays),
    dimreducs = c("pca","refUMAP")
  )
...
ValidateAzimuthReference <- function(object, ad.name = "AzimuthReference") {
  ...
  if (!"pca" %in% Reductions(object = object)) {
    stop("Object must contain a DimReduc called pca to use in transfer/projection.")
  }
...
ConvertGeneNames_1 <- function (object, reference.names, homolog.table) {
  linked <- readRDS(file = homolog.table)
  query.names <- rownames(x = object)
  
  # Detect if the IDs are Ensembl or gene names
  ensembl.id <- "(?:ENSG|ENSMUS)"
  capture.nonversion <- paste0("(", ensembl.id, ".*)\\.(?:.*)")
  pattern <- paste0("(?:", capture.nonversion, ")|(.*)")
  
  query.names <- Filter(f = function(x) !is.na(x = x), x = as.vector(x = t(x = str_match(string = query.names, pattern = pattern)[, 2:3])))
  
  # Identify if the query IDs are gene names or Ensembl
  query.names.sub <- sample(x = query.names, size = min(length(x = query.names), 5000))
  idtype <- names(x = which.max(x = apply(X = linked, MARGIN = 2, FUN = function(col) {
  ...
  # Convert the reference names to gene names using homologs
  reference.names.sub <- sample(x = reference.names, size = min(length(x = reference.names), 5000))
  idtype.ref <- names(x = which.max(x = apply(X = linked, MARGIN = 2, FUN = function(col) {
  ...
  # If query and reference IDs match, return the object as is
  if (totalid == totalid.ref) {
    return(object)
  } else {
    display.names <- setNames(c("ENSEMBL gene", "ENSEMBL transcript", "gene name", "transcript name"), nm = c("Gene.stable.ID", "Transcript.stable.ID", "Gene.name", "Transcript.name"))
    
    linked.unique <- linked[!duplicated(x = linked[[totalid]]), ]
    new.indices <- which(query.names %in% linked.unique[[totalid]])
    
    message("Found ", length(x = new.indices), " out of ", length(x = query.names), " total inputs in conversion table")
